...
#
# arty-uart-memory.tcl: Tcl script for re-creating project 'arty-uart-memory'
#
#*****************************************************************************************
...
proc checkRequiredFiles { origin_dir} {
  set status true
  set files [list \
 "[file normalize "$origin_dir/src/v/debounce.sv"]"\
 "[file normalize "$origin_dir/src/v/led_pwm.sv"]"\
 "[file normalize "$origin_dir/src/v/axi4_lite_addr_control.sv"]"\
 "[file normalize "$origin_dir/src/v/bp_cache_dma_to_axi4_lite.sv"]"\
 "[file normalize "$origin_dir/src/v/bp_bedrock_to_axi4_lite.sv"]"\
 ...
 "[file normalize "$origin_dir/src/v/bp_fpga_host.sv"]"\
 "[file normalize "$origin_dir/src/v/arty_uart_memory.sv"]"\
 "[file normalize "$origin_dir/src/xdc/constraints_bp.xdc"]"\
 "[file normalize "$origin_dir/src/tcl/design_1_bp.tcl"]"\
  ]
  foreach ifile $files {
...
set _xil_proj_name_ "arty-uart-memory"
...
set script_file "arty-uart-memory.tcl"
...
set blackparrot_dir "$origin_dir/blackparrot/rtl"
...
set_property -name "board_part_repo_paths" -value "[file normalize "$origin_dir/common/board_files"]" -objects $obj
...
# Create design_1
set design_tcl "${origin_dir}/src/tcl/design_1_bp.tcl"
source ${design_tcl}
...
proc load_bp_sources_from_flist { origin_dir blackparrot_dir } {
  ...
  set flist_path "${origin_dir}/src/flist/bp.flist"
...
set additional_include_dirs [list \
  [file normalize "${origin_dir}/src/include/" ] \
]
...
set obj [get_filesets sources_1]
set_property -name "top" -value "arty_uart_memory" -objects $obj
...
set file "[file normalize "$origin_dir/src/xdc/constraints_bp.xdc"]"
set file_added [add_files -norecurse -fileset constrs_1 [list $file]]
set file "${origin_dir}/src/xdc/constraints_bp.xdc"
